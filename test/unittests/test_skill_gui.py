...
from ovos_workshop.skills.base import SkillGUI, BaseSkill
from ovos_utils.messagebus import Message, FakeBus

#from ovos_utils.file_utils import resolve_resource_file
# TODO - resolve_resource_file needs to account for res being split across packages now

def resolve_resource_file(page):
    return f'/test/{page}'


class TestSkillGUI(TestCase):
    def setUp(self):
        self.mock_skill = BaseSkill(name='Skill')
        self.mock_skill.bind(FakeBus())
        self.mock_skill.skill_id = 'fortytwo-skill'

        self.mock_skill._resources.locate_qml_file = resolve_resource_file
    ...
    @unittest.skip("TODO fix bus mocks")
    ...
    def test_connected_no_response(self):
        ...
        response = None
        self.mock_skill.bus.wait_for_response = lambda k: None
        self.assertFalse(self.gui.connected)
